...
{
    public abstract class BaseIntegerType<TIntegerType> : ScalarType
        where TIntegerType : IComparable
    {
        protected abstract TIntegerType MinValue { get; }
        protected abstract TIntegerType MaxValue { get; }

        public BaseIntegerType(string name)
            : base(name)
        {
            this.RuntimeType = typeof(TIntegerType);
        }
        ...
        public override bool IsOfType(IGraphValue value) =>
            value switch
            {
                ...
                GraphFloatValue _value when Math.Truncate(_value.Value) == _value.Value
                    => _value.Value.CompareTo(this.MinValue) >= 0 && _value.Value.CompareTo(this.MaxValue) <= 0,
    ...
    public abstract class BaseIntegerTypeResolver<TIntegerType> : ILeafValueResolver
        where TIntegerType : IComparable
    {
        ...
        public IGraphValue ResolveValue(object? value) =>
            value switch
            {
                TIntegerType _value => new GraphFloatValue(Convert.ToDouble(_value)),
        ...
        public object? CoerceResult(object? value) =>
            value switch
            {
                TIntegerType => Convert.ToDouble(value),
                float => value,
                double => value,
