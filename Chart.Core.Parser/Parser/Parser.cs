namespace Chart.Core.Parser
{
    ...
    public partial class Parser
    {
        ...
        private void DecreaseDepth()
        {
            ...
            {
                throw new UnmatchedBraceException(this.CurrentToken);
            }
        }
        ...
        /// <summary>
        /// Find a matching token in the document and return the index.
        /// </summary>
        /// <param name="start">The opening token-type to match.</param>
        /// <param name="end">The closing token-type to match.</param>
        /// <returns>The token index to the matching token, if successful. Otherwise, -1.</returns>
        private int FindMatching(TokenType start, TokenType end)
        {
            this.Expect(start);

            int iter = this.CurrentTokenIndex;
            int depth = 0;

            do
            {
                Token token = this.Tokens[iter];
        ...
        public GraphDocument Parse(string source, List<Token> tokens, ParserOptions options)
        {
            this.Tokens = tokens;
        ...
        public GraphDocument Parse(string source)
            => this.Parse(source, new Tokenizer(source).GetAllTokens(), new ParserOptions());
        ...
        /// <exception cref="MissingBraceException">Thrown when the number of braces isn't even.</exception>
        /// <exception cref="UnexpectedTokenException">Thrown when an unexpected token was found.</exception>
        public GraphDocument Parse(string source, ParserOptions options)
        ...
        public GraphDocument Parse(string source, List<Token> tokens)
            => this.Parse(source, tokens, new ParserOptions());
